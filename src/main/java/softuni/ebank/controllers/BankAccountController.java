...
public class BankAccountController {
    ...
    public String createBankAccount(@ModelAttribute("bankAccountBindingModel") BankAccountBindingModel bankAccountBindingModel, Principal principal, Model model){
    ...
    @PostMapping("/create")
    @PreAuthorize("isAuthenticated()")
    public String createBankAccountConfirm(@ModelAttribute("bankAccountBindingModel") BankAccountBindingModel bankAccountBindingModel, Model model){
    ...
    private boolean withdrawAmount(BankAccountBindingModel bankAccountBindingModel) {
        BankAccount bankAccount = this.bankAccountRepository.findById(bankAccountBindingModel.getId())
                .orElse(null);
        if(bankAccount == null || bankAccountBindingModel.getAmount().compareTo(BigDecimal.ZERO) <= 0 ||
           bankAccount.getBalance().compareTo(bankAccountBindingModel.getAmount()) < 0){
            return false;
        }
